...
def dig_images(filepath):
  ...
  Returns array of shape 60000x28x28x1 ready for feeding to NN.
  ...
  train_matrix = ~np.ctypeslib.as_array(matrix, shape=(int.from_bytes(bin_digits[4:8], byteorder='big'),) ).reshape( int.from_bytes(bin_digits[4:8], byteorder='big'), 28,28,1) / 255.0
